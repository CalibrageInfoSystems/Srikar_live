...
class _MyReturnOrdersPageState extends State<ViewReturnorder> {
  ...
  void didChangeDependencies() {
    ...
    returnOrdersProvider =
        Provider.of<ViewReturnOrdersProvider>(context, listen: false);
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['response']['listResult'] != null) {
          List<dynamic> listResult = jsonResponse['response']['listResult'];
          List<ReturnOrdersList> result = listResult
              .map((item) => ReturnOrdersList.fromJson(item))
              .toList();
          returnOrdersProvider.fetchReturnOrders = result;
          return result;
        } else {
          returnOrdersProvider.fetchReturnOrders = [];
          return [];
  ...
  filterRecordsBasedOnPartyName(String input) {
    apiData.then((data) {
      setState(() {
        returnOrdersProvider.fetchReturnOrders = data
            .where((item) =>
...
class _FilterBottomSheetState extends State<FilterBottomSheet> {
  ...
  Future<void> getappliedfilterData() async {
    viewReturnOrdersProvider.filterStatus = true;
    int companyId = await SharedPrefsData.getIntFromSharedPrefs("companyId");
    String userId = await SharedPrefsData.getStringFromSharedPrefs("userId");
    String apiurl = baseUrl + GetAppReturnOrdersBySearch;
    ...
    try {
      ...
      debugPrint('_______Return Orders____2___${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['response']['listResult'] != null) {
          ...
          viewReturnOrdersProvider.fetchReturnOrders = result;
        } else {
          List<ReturnOrdersList> emptyList = [];
          viewReturnOrdersProvider.fetchReturnOrders = emptyList;
        }
      } else {
        List<ReturnOrdersList> emptyList = [];
        viewReturnOrdersProvider.fetchReturnOrders = emptyList;
