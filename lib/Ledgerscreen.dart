...
import 'Common/ProgressDialog.dart';
...
class Ledger_screen extends State<Ledgerscreen> {
          ...
          if (jsonResponse['result']['isSuccess']) {
            if (jsonResponse['result']['response'] == null) {
              ...
              if (await file.exists()) {
                ...
                await file.delete();
                print('File deleted.');
                // Adding a small delay to ensure file system updates
              ...
              if (!await file.exists()) {
                ...
                // If deletion still failed, attempt to rename the existing file
                String tempFileName = "${companyCode}_${widget.cardCode}_${pdffromdate}_$pdftodate-temp.pdf";
                final File tempFile = File('$filePath/$tempFileName');
                print('File still exists, renaming to temp file...');
                await file.rename(tempFile.path);
                print('Renamed old file to temp file.');
  ...
  //       // Your existing downloadData function code goes here
  //
  //       // Example code to show selected download path
  //       showDialog(
  //         context: context,
  //         builder: (BuildContext context) {
  //           return AlertDialog(
  //             title: Text('Selected Directory'),
  //             content: Text('Download Path: $downloadPath'),
  //             actions: <Widget>[
  //               TextButton(
  //                 onPressed: () {
  //                   Navigator.of(context).pop();
...
setState(() {
  progressDialog.dismiss();
          ...
          progressDialog.dismiss();
          print('else Error: ${response.statusMessage}');
        }
      } catch (error) {
        progressDialog.dismiss();
        print('catchError: $error');
  ...
  //
  //         final File file = File('$filePath/$fileName');
  //      //   final File file = File('$filePath/example.pdf');
  //         await file.create(recursive: true);
  //         await file.writeAsBytes(pdfBytes);
  //         CommonUtils.showCustomToastMessageLong(
  //             'Ledger Report Downloaded Successfully', context, 0, 4);
  //         // final directory = await getDownloadsDirectory();
  //         // final filePath = path.join(directory!.path, 'downloaded_file.pdf');
